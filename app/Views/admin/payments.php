...
function renderPayments(payments) {
    ...
    for (var i = 0; i < payments.length; i++) {
        var p = payments[i];
        const statusClass = p.status === 'completed' ? 'bg-green-100 text-green-800' : 
        ...
        html += '<tr class="border-b hover:bg-gray-50 transition">' +
            '<td class="px-4 py-3" style="color: #111827;">' + (p.id || 'N/A') + '</td>' +
            '<td class="px-4 py-3" style="color: #111827;">' + (p.application_id || 'N/A') + '</td>' +
            '<td class="px-4 py-3" style="color: #111827;">₹' + (p.amount || 0) + '</td>' +
            '<td class="px-4 py-3"><span class="px-2 py-1 rounded text-xs font-semibold ' + statusClass + '">' + (p.status || 'pending') + '</span></td>' +
            '<td class="px-4 py-3 text-xs" style="color: #6B7280;">' + date + '</td>' +
            '<td class="px-4 py-3">' +
            '<div class="flex items-center gap-2">' +
            '<a href="/admin/payments/' + p.id + '" class="p-1.5 bg-blue-100 text-blue-600 rounded hover:bg-blue-200 transition" title="<?= esc(lang('App.adminView')) ?>">' +
            '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>' +
            '</a>' +
            '<button onclick="deletePayment(' + p.id + ')" class="p-1.5 bg-red-100 text-red-600 rounded hover:bg-red-200 transition" title="<?= esc(lang('App.adminDelete')) ?>">' +
